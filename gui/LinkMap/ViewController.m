...
- (void)viewDidLoad {
    ...
    self.methodArray = [NSMutableArray new];
        ...
        if([line hasPrefix:@"#"]) {
            ...
            if(reachFiles == YES && reachSections == NO && reachSymbols == NO) {
                ...
                if(range.location != NSNotFound) {
                    ...
                    symbol.methodArray = [NSMutableArray new];
                ...
                if(symbolsArray.count == 3) {
                    ...
                    if(range.location != NSNotFound) {
                        ...
                        MethodModel *model = [MethodModel new];
                        model.methodName = fileKeyAndName;
                        model.methodSize = size;
                        
                        
                        
                        [symbol.methodArray addObject:model];
                        
//                        [self.methodArray addObject:model];_methodArray	__NSArrayM *	@"0 elements"	0x000060800044ec70
                        if(symbol) {
                            symbol.size += size;
                        }
                    }
...
- (void)buildResultWithSymbols:(NSArray *)symbols {
    ...
    NSMutableArray <SymbolModel *> *symbolArray = [NSMutableArray new];
...
//    NSInteger index = 0;
    for(SymbolModel *symbol in symbolArray) {
        if (searchKey.length > 0) {
            if ([symbol.file containsString:searchKey]) {
//                if (startIndex == 0) {
////                 startIndex = index;
...
- (void)appendResultWithSymbol:(SymbolModel *)model {
    ...
    model.methodArray = [self sortMethodModel:model.methodArray];


    for (MethodModel *mothed in model.methodArray) {
        
        if (mothed.methodSize / 1024.0 / 1024.0 > 1) {
            methodsize = [NSString stringWithFormat:@"%.2fM",mothed.methodSize/ 1024.0 / 1024.0];
        } else {
            methodsize = [NSString stringWithFormat:@"%.2fK", mothed.methodSize / 1024.0];
        }
...
- (NSMutableArray *)sortMethodModel:(NSArray <MethodModel *>*) methodModels {
    NSArray *sortedSymbols = [methodModels sortedArrayUsingComparator:^NSComparisonResult(MethodModel *  _Nonnull obj1, MethodModel *  _Nonnull obj2) {
        if(obj1.methodSize > obj2.methodSize) {
            return NSOrderedAscending;
        } else if (obj1.methodSize < obj2.methodSize) {
            return NSOrderedDescending;
        } else {
            return NSOrderedSame;
        }
    }];
    NSMutableArray *array =[NSMutableArray arrayWithArray:sortedSymbols];
    return  array;
//    return sortedSymbols;
}
